 ...
 * Permet de définir des routes HTTP et d'exécuter des actions correspondantes 
 * en fonction des requêtes entrantes
 */
class Router{
    //Propriété privée pourstocker les routes enregistrées
    private $routes = [];
    ...
    /**
     * Ajoute une route à la lilste des routes.
     * 
     ...
     * @return void
     */
function addRoute(string $method,string $url,callable $action){
    $this->routes[] = [
        "method"=> $method,
        "url"=> $url,
        "action"=> $action
    ...
    /**
     * Résout la requête entrante en fonction des routes enregistées
     * 
     * - Compare l'URL er la méthode HTTP de la requête avec chaque route enregistrée.
     * - Si une correspondance est trouvée, l'acton associée est exécutée.
